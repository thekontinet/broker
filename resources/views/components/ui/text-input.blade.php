...
<div>
    ...
    <div class="relative">
        <input id="{{$id}}" {{$attributes->class(['pr-11' => $suffix && !str_contains($attributes->get('class'), 'pr-'), 'pl-11' => $prefix && !str_contains($attributes->get('class'), 'pl-'), "py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"])}}  @disabled($disabled)>
        @if($prefix)
